...
<div class="mb-8 bg-gradient-to-r from-blue-600 to-blue-700 rounded-2xl p-8 text-white shadow-xl">
    <div class="flex flex-col md:flex-row justify-between items-start md:items-center gap-4">
        ...
        <a href="{{ route('admin.berita.create') }}" class="bg-white text-blue-700 hover:bg-blue-50 rounded-xl px-5 py-2.5 text-sm font-semibold transition-all inline-flex items-center gap-2 shadow-lg">
    ...
    <div class="bg-white rounded-xl p-5 shadow-lg border border-blue-100">
        <div class="flex items-center justify-between">
            <div>
                ...
                <p class="text-2xl font-extrabold mt-1 text-blue-600">{{ $berita->total() }}</p>
            </div>
    ...
    <div class="bg-white rounded-2xl shadow-lg overflow-hidden border border-blue-100 hover:shadow-xl transition-all transform hover:-translate-y-1">
        ...
        <div class="p-5">
            ...
            <div class="flex space-x-2">
                <a href="{{ route('admin.berita.edit', $item->id) }}" class="flex-1 text-center bg-blue-50 text-blue-600 px-4 py-2.5 rounded-xl hover:bg-blue-100 transition font-semibold text-sm inline-flex items-center justify-center gap-1">
                    <iconify-icon icon="mdi:pencil" class="text-sm"></iconify-icon> Edit
                </a>
                <form id="delete-form-{{ $item->id }}" method="POST" action="{{ route('admin.berita.destroy', $item->id) }}" class="flex-1">
                    @csrf
                    @method('DELETE')
                    <button type="button" onclick="if(confirm('Yakin ingin menghapus berita ini?')) document.getElementById('delete-form-{{ $item->id }}').submit();" class="w-full bg-red-50 text-red-600 px-4 py-2.5 rounded-xl hover:bg-red-100 transition font-semibold text-sm border border-red-200 inline-flex items-center justify-center gap-1">
