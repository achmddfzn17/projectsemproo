...
@section('title', 'Detail Hasil SUS - ' . $hasil->nama)

@section('content')
<!-- Back Button -->
<div class="mb-6">
    <a href="{{ route('admin.sus.daftar') }}" class="inline-flex items-center gap-2 text-gray-600 hover:text-gray-800 transition-colors">
        ...
        <div class="relative z-10 flex flex-col lg:flex-row items-center justify-between gap-8">
            ...
            <div class="text-center lg:text-left">
                ...
                <div class="flex items-baseline gap-3 justify-center lg:justify-start">
                    <span class="text-7xl lg:text-8xl font-black">{{ number_format($hasil->sus_score, 1) }}</span>
                ...
                <div class="mt-4 flex items-center gap-3 justify-center lg:justify-start">
                    <span class="inline-flex items-center px-4 py-2 rounded-full text-sm font-bold {{ $gc['bg'] }} text-white shadow-lg">
                        Grade {{ $grade }}
                    </span>
                    <span class="text-xl font-semibold text-white/90">{{ $gradeLabel }}</span>
                </div>
                <p class="mt-2 text-blue-100">{{ $gradeDesc }}</p>
            </div>
            ...
            <div class="relative w-48 h-48 flex-shrink-0">
                ...
                <div class="absolute inset-0 flex flex-col items-center justify-center">
                    <span class="text-4xl font-black">{{ number_format($hasil->sus_score, 0) }}</span>
            ...
            <div class="p-6">
                ...
                <div class="flex items-center gap-4 mb-6 pb-6 border-b border-gray-100">
                    <div class="w-16 h-16 rounded-2xl bg-gradient-to-br from-blue-500 to-indigo-600 flex items-center justify-center text-white text-2xl font-bold shadow-lg">
                        {{ substr($hasil->nama, 0, 1) }}
                    </div>
                    <div>
                        <h4 class="font-bold text-gray-800 text-lg">{{ $hasil->nama }}</h4>
                        <p class="text-gray-500 text-sm">{{ $hasil->email }}</p>
                    </div>
                </div>
                ...
                <div class="space-y-4">
                    <div class="flex items-center gap-3">
                        ...
                        <div>
                            ...
                            <p class="font-semibold text-gray-800">{{ $hasil->usia }} tahun</p>
                        </div>
                    </div>
                    ...
                    <div class="flex items-center gap-3">
                        ...
                        <div>
                            ...
                            <p class="font-semibold text-gray-800">{{ $hasil->pendidikan }}</p>
                        </div>
                    </div>
                    ...
                    <div class="flex items-center gap-3">
                        ...
                        <div>
                            ...
                            <p class="font-semibold text-gray-800">{{ $hasil->pengalaman_komputer }}</p>
                        </div>
                    </div>
                </div>
            </div>
            ...
            <div class="p-6">
                ...
                <div class="space-y-4">
                    ...
                    <div class="flex items-center gap-4 p-4 rounded-xl border border-gray-100 hover:border-blue-200 hover:bg-blue-50/30 transition-all">
                        <!-- Question Number -->
                        <div class="w-10 h-10 rounded-xl {{ $q['positif'] ? 'bg-blue-100 text-blue-600' : 'bg-purple-100 text-purple-600' }} flex items-center justify-center font-bold text-sm flex-shrink-0">
                            {{ $i }}
                        </div>
                        
                        <!-- Question Text -->
                        <div class="flex-1 min-w-0">
                            <p class="font-medium text-gray-800 text-sm leading-relaxed">{{ $q['text'] }}</p>
                            <div class="flex items-center gap-2 mt-1">
                                <span class="text-xs text-gray-500">{{ $q['positif'] ? 'Positif' : 'Negatif' }}</span>
                                <span class="text-gray-300">•</span>
                                <span class="text-xs text-gray-500">Skor: {{ $skor }}/4</span>
                            </div>
                        </div>
                        ...
                        <div class="flex items-center gap-3 flex-shrink-0">
                            ...
                            <div class="hidden sm:flex items-center gap-2">
                                <div class="w-16 bg-gray-100 rounded-full h-2 overflow-hidden">
                                    <div class="h-full rounded-full transition-all @if($skor >= 3) bg-green-500 @elseif($skor >= 2) bg-yellow-500 @else bg-red-500 @endif" style="width: {{ $skor * 25 }}%"></div>
                                </div>
                            </div>
                            
                            <!-- Answer Badge -->
                            <div class="flex flex-col items-center">
                                <span class="w-10 h-10 rounded-xl {{ $statusColor }} border flex items-center justify-center font-bold text-lg">
                                    {{ $jawaban }}
                                </span>
                                <iconify-icon icon="{{ $skor >= 2 ? 'mdi:check-circle' : 'mdi:close-circle' }}" class="{{ $iconColor }} text-lg mt-1"></iconify-icon>
                            </div>
                        </div>
                    </div>
                    @endfor
                ...
                <div class="mt-6 pt-6 border-t border-gray-200 flex items-center justify-between">
                    <div class="flex items-center gap-3">
                        ...
                        <div>
                            ...
                            <p class="font-bold text-gray-800">{{ $hasil->total_score }} / 40</p>
                        </div>
                    </div>
                    <div class="flex items-center gap-3">
                        ...
                        <div class="text-right">
                            ...
                            <p class="font-bold text-xl text-indigo-600">{{ number_format($hasil->sus_score, 1) }} / 100</p>
                        </div>
                    </div>
                </div>
            </div>
...
<div class="bg-white rounded-2xl shadow-lg border border-gray-100 overflow-hidden mb-8">
    ...
    <div class="p-6">
        <div class="grid grid-cols-2 md:grid-cols-5 gap-4">
            ...
            <div class="p-4 rounded-xl border-2 {{ $borderColor }} {{ $bgColor }} text-center transition-all relative {{ $isActive ? 'ring-2 ring-offset-2 ring-' . $s['color'] . '-400' : '' }}">
                @if($isActive)
                <div class="absolute -top-3 left-1/2 -translate-x-1/2 bg-{{ $s['color'] }}-500 text-white text-xs font-bold px-3 py-1 rounded-full">
                    Anda Disini
                </div>
                @endif
                <span class="inline-block w-12 h-12 rounded-full bg-{{ $s['color'] }}-100 text-{{ $s['color'] }}-600 font-black text-xl mb-2 flex items-center justify-center mx-auto">{{ $s['grade'] }}</span>
                <p class="font-bold text-gray-800 text-sm">{{ $s['label'] }}</p>
                <p class="text-xs text-gray-500">{{ $s['desc'] }}</p>
                <p class="text-xs font-medium text-{{ $s['color'] }}-600 mt-1">{{ $s['range'] }}</p>
            </div>
            @endforeach
        </div>
    </div>
...
<div class="flex flex-col sm:flex-row items-center justify-between gap-4">
    <a href="{{ route('admin.sus.daftar') }}" class="w-full sm:w-auto flex items-center justify-center gap-2 bg-gray-100 hover:bg-gray-200 text-gray-700 font-semibold py-3 px-6 rounded-xl transition-all">
        <iconify-icon icon="mdi:arrow-left" class="text-xl"></iconify-icon>
        <span>Kembali ke Daftar</span>
    </a>
    
    <div class="flex items-center gap-3 w-full sm:w-auto">
        <form action="{{ route('admin.sus.destroy', $hasil->id) }}" method="POST" class="flex-1 sm:flex-none">
            @csrf
            @method('DELETE')
        ...
        <a href="{{ route('admin.sus.index') }}" class="flex-1 sm:flex-none flex items-center justify-center gap-2 bg-gradient-to-r from-blue-600 to-indigo-600 hover:from-blue-700 hover:to-indigo-700 text-white font-semibold py-3 px-6 rounded-xl transition-all shadow-lg hover:shadow-xl transform hover:-translate-y-0.5">
