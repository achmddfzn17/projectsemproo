    ...
    <div class="relative z-10">
        <div class="flex flex-col md:flex-row justify-between items-start md:items-center gap-4">
            ...
            <div class="flex gap-3">
                <a href="{{ route('admin.sus.index') }}" class="bg-white/20 hover:bg-white/25 backdrop-blur-sm rounded-2xl px-5 py-3 text-sm font-medium transition-all flex items-center gap-3 hover:shadow-lg transform hover:scale-[1.02]">
                    <iconify-icon icon="mdi:plus-circle" class="text-xl"></iconify-icon>
                    <span>Kuisioner Baru</span>
                </a>
                @if($totalResponden > 0)
                <a href="{{ route('admin.sus.export-pdf') }}" class="bg-white/20 hover:bg-white/25 backdrop-blur-sm rounded-2xl px-5 py-3 text-sm font-medium transition-all flex items-center gap-3 hover:shadow-lg transform hover:scale-[1.02]">
...
<div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6 mb-8">
    ...
    <div class="bg-white rounded-2xl shadow-lg border border-blue-100 p-6 hover:shadow-xl transition-all hover:-translate-y-1">
        ...
        <h3 class="text-4xl font-bold text-gray-800 mb-1">{{ $totalResponden }}</h3>
    ...
    <div class="bg-white rounded-2xl shadow-lg border border-green-100 p-6 hover:shadow-xl transition-all hover:-translate-y-1">
        <div class="flex items-center justify-between mb-4">
            ...
            @endphp
            <span class="text-xs font-semibold px-3 py-1 rounded-full {{ $scoreColor }}">{{ $scoreLabel }}</span>
        </div>
        <h3 class="text-4xl font-bold text-gray-800 mb-1">{{ $rataRataSkor }}</h3>
    ...
    <div class="bg-white rounded-2xl shadow-lg border border-purple-100 p-6 hover:shadow-xl transition-all hover:-translate-y-1">
        <div class="flex items-center justify-between mb-4">
            ...
            @endphp
            <span class="text-xs font-semibold px-3 py-1 rounded-full {{ $overallColor }}">{{ $overallLabel }}</span>
        </div>
        <h3 class="text-2xl font-bold text-gray-800 mb-1">{{ $overallLabel }}</h3>
    ...
    <div class="bg-white rounded-2xl shadow-lg border border-indigo-100 p-6 hover:shadow-xl transition-all hover:-translate-y-1">
        ...
        <h3 class="text-4xl font-bold text-gray-800 mb-1">{{ $persenLayak }}%</h3>
        ...
        <div class="space-y-4">
            ...
            <div class="flex items-center gap-4">
                <div class="w-16 text-center">
                    <span class="text-2xl font-bold {{ $g['text'] }}">{{ $g['grade'] }}</span>
                </div>
                <div class="flex-1">
                    <div class="flex justify-between items-center mb-1">
                        <span class="text-sm font-medium text-gray-700">{{ $g['label'] }}</span>
                        <span class="text-sm text-gray-500">{{ $gradeCounts[$g['grade']] }} responden ({{ $totalResponden > 0 ? round(($gradeCounts[$g['grade']] / $totalResponden) * 100, 1) : 0 }}%)</span>
                    </div>
                    <div class="w-full bg-gray-100 rounded-full h-3 overflow-hidden">
                        <div class="{{ $g['color'] }} h-full rounded-full transition-all duration-500 ease-out sus-bar"
                             data-width="{{ $totalResponden > 0 ? ($gradeCounts[$g['grade']] / $maxCount) * 100 : 0 }}"></div>
                    </div>
                </div>
            </div>
            @endforeach
...
<div class="bg-white rounded-2xl shadow-lg border border-blue-100 overflow-hidden">
    <div class="p-6 border-b border-blue-100">
        <div class="flex flex-col md:flex-row justify-between items-start md:items-center gap-4">
            ...
            <span class="text-sm text-gray-500">{{ $totalResponden }} total responden</span>
        </div>
    </div>
    ...
    <div class="overflow-x-auto">
        <table class="w-full">
            ...
            <tbody class="divide-y divide-gray-100">
                @foreach($hasilSus as $index => $item)
                @php
                ...
                <tr class="hover:bg-blue-50/50 transition-colors">
                    ...
                    <td class="px-6 py-4 text-sm text-gray-600">{{ $item->usia }} th</td>
                    <td class="px-6 py-4 text-sm text-gray-600">{{ $item->pendidikan }}</td>
                    <td class="px-6 py-4 text-center">
                        <span class="font-bold {{ $scoreColor }} text-lg">{{ number_format($item->sus_score, 1) }}</span>
                    ...
                    <td class="px-6 py-4 text-center">
                        <div class="flex items-center justify-center gap-2">
                            <a href="{{ route('admin.sus.hasil', $item->id) }}" class="p-2 rounded-xl bg-blue-100 text-blue-600 hover:bg-blue-200 transition-colors" title="Lihat Detail">
                                <iconify-icon icon="mdi:eye" class="text-lg"></iconify-icon>
                            </a>
                                <form id="delete-form-{{ $item->id }}" action="{{ route('admin.sus.destroy', $item->id) }}" method="POST">
                                    @csrf
                                    @method('DELETE')
    ...
    <div class="p-12 text-center">
        ...
        <a href="{{ route('admin.sus.index') }}" class="inline-flex items-center gap-2 bg-gradient-to-r from-blue-600 to-indigo-600 hover:from-blue-700 hover:to-indigo-700 text-white font-bold py-3 px-6 rounded-xl transition-all transform hover:-translate-y-1 shadow-lg">
            <iconify-icon icon="mdi:plus-circle" class="text-xl"></iconify-icon>
