...
@section('title', 'Galeri - Karang Taruna Generasi Emas')
    ...
    <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
            ...
            <div class="group relative overflow-hidden rounded-2xl shadow-lg bg-gray-100 aspect-square">
                ...
                <div class="absolute inset-0 bg-gradient-to-t from-black/70 via-black/20 to-transparent opacity-0 group-hover:opacity-100 transition-all duration-300 flex flex-col justify-end p-6">
                    ...
                    @if($item->kegiatan)
                    <p class="text-blue-200 text-sm mb-2">{{ $item->kegiatan->nama_kegiatan }}</p>
                    @endif
                    @if($item->deskripsi)
                    <p class="text-gray-200 text-sm line-clamp-2">{{ $item->deskripsi }}</p>
                    @endif
                    <p class="text-gray-300 text-xs mt-2">{{ $item->created_at->format('d F Y') }}</p>
                </div>
        ...
        <div class="mt-12 flex justify-center">
            <nav class="flex items-center space-x-2">
                ...
                @else
                <a href="{{ $galeris->previousPageUrl() }}" class="px-4 py-2 bg-blue-500 text-white rounded-lg hover:bg-blue-600 transition">
                    <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M15 19l-7-7 7-7"></path>
                    </svg>
                </a>
                ...
                <span class="px-4 py-2 bg-gray-100 text-gray-700 rounded-lg">
                    Halaman {{ $galeris->currentPage() }} dari {{ $galeris->lastPage() }}
                </span>
                
                @if($galeris->hasMorePages())
                <a href="{{ $galeris->nextPageUrl() }}" class="px-4 py-2 bg-blue-500 text-white rounded-lg hover:bg-blue-600 transition">
                    <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
        ...
        <div class="text-center py-20">
            ...
            <a href="{{ route('home') }}" class="inline-flex items-center gap-2 bg-blue-500 hover:bg-blue-600 text-white px-6 py-3 rounded-xl font-semibold transition">
                <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
...
<section class="bg-gradient-to-r from-blue-500 to-blue-600 py-12">
    <div class="max-w-4xl mx-auto px-4 text-center">
        ...
        <a href="{{ route('kegiatan.index') }}" class="inline-flex items-center gap-2 bg-white text-blue-600 px-8 py-4 rounded-xl font-bold hover:bg-blue-50 transition shadow-lg">
            <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
