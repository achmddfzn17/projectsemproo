    ...
    <div class="max-w-2xl mx-auto">
        ...
        <div class="bg-white rounded-2xl shadow-lg p-6 border border-blue-100 mb-6">
            ...
            <div class="space-y-3">
                <div class="flex justify-between">
                    ...
                    <span class="font-bold text-blue-600">#{{ str_pad($pendaftaran->nomor_urut, 4, '0', STR_PAD_LEFT) }}</span>
                </div>
                <div class="flex justify-between">
                    <span class="text-gray-600">Nama:</span>
                    <span class="font-semibold">{{ $pendaftaran->nama_lengkap }}</span>
                </div>
                <div class="flex justify-between">
                    <span class="text-gray-600">Email:</span>
                    <span class="font-semibold">{{ $pendaftaran->email }}</span>
                </div>
                <div class="flex justify-between">
                    <span class="text-gray-600">Kegiatan:</span>
                    <span class="font-semibold">{{ $pendaftaran->kegiatan->nama_kegiatan }}</span>
                </div>
                <div class="flex justify-between">
        ...
        <div class="bg-white rounded-2xl shadow-lg p-8 border border-blue-100 text-center mb-6">
            ...
            <div class="inline-block bg-white p-6 rounded-xl border-4 border-blue-200 mb-4">
                <img src="https://api.qrserver.com/v1/create-qr-code/?size=300x300&data={{ urlencode($pendaftaran->qr_token) }}" 
                     alt="QR Code" 
                     class="w-64 h-64">
            </div>

            <p class="text-sm text-gray-500 mb-4">
                Nomor: #{{ str_pad($pendaftaran->nomor_urut, 4, '0', STR_PAD_LEFT) }}
            </p>
        ...
        <div class="flex space-x-4">
            <a href="{{ route('kegiatan.detail', $pendaftaran->kegiatan_id) }}" 
...
function downloadQR() {
    ...
    link.download = 'qr_pendaftaran_{{ $pendaftaran->nomor_urut }}.png';
    link.click();
}
</script>
